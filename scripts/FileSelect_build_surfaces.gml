...
{   // ---------------------------------------------------
    ...
    if(!surface_exists(MAIN_surf))
    {
        ...
        var _i,_j, _idx, _val;
        var _x,_y, _x1,_y1;
        var _DrawArea_XL = MAIN_surf_DrawArea_XL;
        var _DrawArea_XC = viewXC();
        var _DrawArea_YT = MAIN_surf_DrawArea_YT;
        var _X1  = MAIN_surf_CLMS; // Frame XR
            _X1 -= $02; // Frame + Padding
            _X1 -=($04*3); // Levels + Level pad
            _X1  = _DrawArea_XL + (_X1<<3); // Levels XL
        var _Y1  = _DrawArea_YT + ($01<<3); // Frame YT
...
if ((state==State_REGISTER  && !surface_exists(REGISTER_surf)) 
||  (state==State_ELIMINATE && !surface_exists(ELIMINATE_surf)) )
{
    var _i,_j;
    var _idx = 0;
    ...
    // YT of "END"
    switch(state){
    case State_REGISTER: {var _Y2=_Y1+($02<<3); break;}
    case State_ELIMINATE:{var _Y2=_Y1; break;}
    }
    ...
    // HEADER =======================
    switch(state){
    case State_REGISTER: {_text=" REGISTER YOUR NAME "; break;}
    case State_ELIMINATE:{_text=" ELIMINATION MODE "; break;}
    }
    _char_w = 8;
